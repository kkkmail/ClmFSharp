...
module ReactionRates = 
    ...
    let getEeDistr seed (rate : ReactionRate option) (rateEnant : ReactionRate option) = 
        match rate, rateEnant with 
        | Some (ReactionRate r), Some (ReactionRate re) -> 
            (r - re) / (r + re) |> EeDistribution.createCentered seed |> Some
        | _ -> None
    ...
    type ReactionRateProvider (p: ReactionRateProviderParams) =
        ...
        static member defaultCatDestrSimModel (rnd : Random) (m, threshold, mult) (simThreshold, n) =
...

            {
                catDestrSimParam = 
                    {
                        simDestrDistribution = UniformDistribution(rnd.Next(), { threshold = simThreshold; scale = None; shift = None }) |> Uniform
                        aminoAcids = aminoAcids
                    }
        ...
        static member defaultCatRacemSimModel (rnd : Random) (m, threshold, mult) (simThreshold, n) =
...

            {
                catRacemSimParam = 
                    {
                        simRacemDistribution = UniformDistribution(rnd.Next(), { threshold = simThreshold; scale = None; shift = None }) |> Uniform
                        aminoAcids = aminoAcids
                    }
