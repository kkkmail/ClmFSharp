...
module ModelData = 
    ...
    let d1 (x : array<double>) xSum xSumN xSumSquaredN = 
        [|
            0.001 * x.[6] // b | synthesis: food <-> b
            -0.001 * x.[1] // food | synthesis: food <-> b
            0.001 * x.[4] // B | synthesis: food <-> B
            -0.001 * x.[1] // food | synthesis: food <-> B
            0.001 * x.[5] // a | synthesis: food <-> a
            -0.001 * x.[1] // food | synthesis: food <-> a
            0.001 * x.[3] // A | synthesis: food <-> A
            -0.001 * x.[1] // food | synthesis: food <-> A
            0.1 * x.[2] // waste | recycling: waste -> food
        |]
        |> Array.sum
    ...
    let d2 (x : array<double>) xSum xSumN xSumSquaredN = 
        [|
            -0.001 * x.[2] // waste | destruction: b <-> waste
            0.001 * x.[6] // b | destruction: b <-> waste
            -0.001 * x.[2] // waste | destruction: B <-> waste
            0.001 * x.[4] // B | destruction: B <-> waste
            -0.001 * x.[2] // waste | destruction: a <-> waste
            0.001 * x.[5] // a | destruction: a <-> waste
            -0.001 * x.[2] // waste | destruction: A <-> waste
                                        ...
                                        allParams =
                                            [|
                                                    ...
                                                    modelParam = 
                                                        {
                                                            ...
                                                            catDestrRndEeParams = 
                                                                {
                                                                    rateMultiplierDistr = { distributionType = Triangular; distributionParams = { threshold = Some 1E-05; scale = Some 100000.0; shift = None } } |> Distribution |> RateMultDistr
...

                                                {
                                                    modelParam = 
                                                        {
                                                            sedDirDistribution = { distributionType = Triangular; distributionParams = { threshold = Some 1E-05; scale = None; shift = None } } |> Distribution
                                                            forwardScale = Some 100.0
                                                            sedDirRatesEeParam =
                                                                {
                                                                    sedDirRateMultiplierDistr = { distributionType = Triangular; distributionParams = { threshold = Some 1E-05; scale = Some 100.0; shift = None } } |> Distribution |> RateMultDistr
                                                                    eeForwardDistribution = { distributionType = BiDelta; distributionParams = { threshold = None; scale = Some 0.95; shift = None } } |> Distribution |> EeDistribution |> Some
                                                                }
